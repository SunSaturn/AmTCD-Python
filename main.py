...
def open_file():
    file = tk.filedialog.askopenfilename(title='Выбор файла',filetypes=(('Текстовые документы (*.txt)','.txt'),('Все файлы','*.*')))
    if file:
        text_fild.delete('1.0',tk.END)
...
def save_file():
    file = tk.filedialog.asksaveasfilename(filetypes=(('Текстовые документы (*.txt)', '.txt'),('Все файлы', '*.*')))
...
text_fild = tk.Text(f_text,padx=10,pady=10,wrap=tk.WORD,spacing3=10)
text_fild.pack(expand=1,fill=tk.BOTH,side=tk.LEFT)
scroll = tk.Scrollbar(f_text,command=text_fild.yview)
